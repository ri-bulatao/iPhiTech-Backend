        ...
        <div class="row">
            <div class="col-12 mt-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form @submit.prevent="updateCourse" @keydown="courseForm.onKeydown($event)">
                            <!-- Title -->
                            <div class="mb-3 row">
                            ...
                            <!-- Submit Button -->
                            <v-button :loading="courseForm.busy" class="btn btn-primary btn-sm" @click="updateCourse()">
                                Submit
                            </v-button>
...
export default {
    ...
    created(){
        ...
        .then(()=>{
            this.courseForm.keys().forEach(key => {
                this.courseForm[key] = this.course[key]
            })

            this.isVideoEmbed = this.course["is_embed"] ? true : false;
        });
    },

    methods: {
        updateCourse() {
            this.$store.dispatch('courses/updateCourse', { id: this.$route.params.id })
                .then(({success, message}) => {
                    if (success) {
                        ToastSuccess('Course Updated', message)
