<template>
    <div class="row">
        ...
        <div class="col-sm-12 border-bottom mb-3">
            <card :title="$t('Update Leave Application')">
                <form @submit.prevent="submitApplication">
                    ...
                    <div class="row py-3" v-if="form.type == 'offset'">
...
export default {
    ...
    computed: mapGetters({
        users: 'users/users',
        application: 'leave-applications/leave_application'
    }),

    methods: {
        submitApplication() {
            this.$store.dispatch('leave-applications/updateLeaveApplication', this.form)
                .then(res => {
    ...
    mounted() {
        ...
        this.$store.dispatch('leave-applications/fetchLeaveApplication', payload)
            .then(res => {
                this.form.id = this.application.id
                this.form.filed = this.application.filed
                this.form.user_id = this.application.user_id
                this.form.type = this.application.type
                this.form.from = this.application.from
                this.form.to = this.application.to
                this.form.status = this.application.status
                this.form.reason = this.application.reason
                this.form.offset_date = this.application.offset_date
                this.form.offset_time = this.application.offset_time
            })
            .catch(err => ToastError())
    },  

    metaInfo () {
        return { title: this.$t('Update Leave Application') }
    },
}
</script>